...
def run_algorithm(algorithm_name: str, map_data: np.ndarray, start: Tuple[int, int], 
    ...
    if algorithm_name == "Học Tăng Cường (RL)":
        try:
            ...
            if "rl_model" not in st.session_state or not st.session_state.rl_model:
                st.error("❌ Chưa chọn mô hình học tăng cường! Vui lòng chọn một mô hình từ danh sách hoặc tải lên từ máy tính của bạn.")
                return None
            ...
            # Sử dụng chiến lược cân bằng làm mặc định
            ...
            if "rl_model_path" in st.session_state and st.session_state.rl_model_path:
                # Sử dụng đường dẫn mô hình đã lưu trong session state
                model_path = st.session_state.rl_model_path
                print(f"Sử dụng đường dẫn mô hình từ session state: {model_path}")
            else:
                # Fallback: xây dựng đường dẫn từ tên mô hình (cách cũ)
                ...
                if not os.path.exists(model_path):
                    # Thử tìm trong thư mục gốc nếu đường dẫn có chứa thư mục con
            ...
            # Ghi log thông tin
            print(f"Đang tải mô hình RL từ: {model_path}")
            ...
            try:
                ...
                # Nếu model_path là dummy model, bỏ qua lỗi và sử dụng agent mặc định
                if "default_model.zip" in model_path:
                    print(f"Sử dụng agent mặc định vì đang dùng model kiểm thử: {str(e)}")
    ...
    # QUAN TRỌNG: Tính toán lại chi phí và tiền dựa trên đường đi cuối cùng đã được xác thực
    if raw_path and path and len(path) != len(raw_path):
        print(f"Tính lại chi phí cho đường đi đã xác thực: từ {len(raw_path)} xuống {len(path)} điểm")
        
        # Đếm số trạm thu phí và trạm xăng trên đường đi đã được xác thực
        toll_count = 0
        gas_station_count = 0
        ...
        for pos in path:
            try:
                ...
                print(f"Lỗi khi kiểm tra ô {pos}: {str(e)}")
        
        print(f"Đường đi đã xác thực có: {toll_count} trạm thu phí, {gas_station_count} trạm xăng")
        
        # Tính lại chi phí nhiên liệu và tiền
        ...
        # Tính chi phí trạm xăng (chỉ tính phần nhiên liệu đã tiêu thụ)
        # Giả định đơn giản: mỗi lần ghé trạm xăng sẽ đổ đầy bình
        ...
        # Tính lại số tiền đã chi và còn lại
        money_spent = toll_cost + fuel_cost
        stats["money"] = algorithm.initial_money - money_spent
        
        print(f"Đã tính lại chi phí: toll_cost={toll_cost}, fuel_cost={fuel_cost}, money_left={stats['money']}")
    ...
    with config_col:
        ...
        **Hiệu ứng trực quan:**
        ...
        """)
                ...
                # Tải mô hình từ máy tính
                st.markdown("Tải lên mô hình học tăng cường từ máy tính của bạn (.zip)")
                ...
                uploaded_model = st.file_uploader("Chọn file mô hình (.zip)", type=["zip"], key="uploaded_model_file")
                
                if uploaded_model is not None:
                    # Tạo thư mục tạm để lưu file tải lên nếu cần
                    ...
                    st.success(f"✅ Đã tải lên thành công: {uploaded_model.name}")
                    st.info(f"📁 Đường dẫn mô hình: {model_path}")
                    
                    # Cho phép sử dụng mô hình này
                    if st.button("Sử dụng mô hình này", type="primary"):
                        # Lưu thông tin mô hình vào session state
                        ...
                        st.success("✅ Đã cập nhật lựa chọn mô hình!")
                
                # Hiển thị thông tin mô hình đang được sử dụng
                st.markdown("---")
                st.markdown("##### Mô hình đang được sử dụng")
                
                if "rl_model_path" in st.session_state and st.session_state.get("rl_model_path"):
                    st.success(f"✅ Mô hình hiện tại: {os.path.basename(st.session_state.rl_model_path)}")
                else:
                    st.warning("⚠️ Chưa chọn mô hình. Vui lòng tải lên một mô hình từ máy tính của bạn.")
        ...
        with st.expander("📊 Xem thống kê chi tiết", expanded=False):
            ...
            if st.session_state.algorithm == "Học Tăng Cường (RL)" and len(stat_tabs) > 4:
                with stat_tabs[4]:
                    ...
                    st.success("💡 Agent học tăng cường cân bằng giữa thời gian, chi phí và an toàn.")
