...
def render_map_config():
    ...
    def update_map_on_change():
        ...
        if 'toll_ratio' in st.session_state and 'gas_ratio' in st.session_state and 'brick_ratio' in st.session_state and 'random_seed' in st.session_state and 'map_size' in st.session_state:
            ...
            # Tính toán số lượng điểm đặc biệt từ tỷ lệ
            total_cells = map_size * map_size
            # Giả định khu vực hiệu quả để đặt là tổng số ô trừ điểm bắt đầu và kết thúc
            effective_area = max(1, total_cells - 2) 
            
            # Thêm yếu tố ngẫu nhiên nhỏ vào số lượng tính toán
            num_tolls_calc = max(0, int(toll_ratio / 100 * effective_area * random.uniform(0.85, 1.15)))
            num_gas_calc = max(0, int(gas_ratio / 100 * effective_area * random.uniform(0.85, 1.15)))
            num_obstacles_calc = max(0, int(brick_ratio / 100 * effective_area * random.uniform(0.85, 1.15)))

            # Tạo bản đồ mới với các tham số đã cập nhật (sử dụng số lượng đã tính toán)
            # Không cần set seed ở đây để mỗi lần nhấn nút sẽ tạo map ngẫu nhiên khác nhau
        ...
        with st.container():
            ...
            if map_type == "Tạo tự động":
                ...
                brick_ratio = st.slider(
                    ...
                    max_value=40,
                    value=22,
                    help="Tỷ lệ ô không đi được (gạch) trên bản đồ. Giá trị cao hơn tạo bản đồ khó hơn.",
                    key="brick_ratio"
                )
